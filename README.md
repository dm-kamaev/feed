...
```sh
make up; # Open in browser http://localhost:8080/
make down;
make logs;
```
...
This project is a NestJS application built on Fastify, chosen for its high performance. It follows a server-side rendering approach, where the backend generates HTML content directly, often using the htmx library on the frontend for dynamic updates without a full page reload.
...
- **Redis**: A Redis instance is used for two primary purposes:
  ...
  3.  The requirements don't require **strict consistency guarantees**, but Redis has TTL out of the box.
